...
from cntk.ops import abs, argmax, element_select, less, relu, reduce_max, reduce_sum, square,splice
...
class DeepQAgent(object):
    ...
    def __init__(self, input_shape, nb_actions,
        ...
        self.d1_imus = Dense(3, init=he_uniform(scale=0.01))
        self.imus = np.array([1,2,1])
        # Metrics accumulator
...
#            ])
#        @Function
#        @Signature(imgin=Tensor[input_shape],imusin=Tensor[(3,1)])
        def MyModel():
            with default_options(activation=relu, init=he_uniform()):
                c1 = Convolution2D((8,8),16,strides=4)
                c2 = Convolution2D((4,4),32,strides=2)(c1)
                c3 = Convolution2D((3,3),32,strides=1)(c2)
                d1_cnn = Dense(256, init=he_uniform(scale=0.01))(c3)
                d1_imus = Dense(7, init=he_uniform(scale=0.01))
                d1 = splice(d1_cnn,d1_imus,axis=0)
                #d1 = splice(d1_cnn,self.d1_imus,axis=0)
                #d1 = d1_cnn
                d2 = Dense(nb_actions,activation=None,init=he_uniform(scale=0.01))(d1)
            #d2 = Dense(5,activation=None,init=he_uniform(scale=0.01))(d1)
            return d2
        print(input_shape)
        imgin = np.zeros([input_shape[0],input_shape[1],input_shape[2]])
        imusin = np.zeros(3)
        self._action_value_net = MyModel()
        self._action_value_net.update_signature(Tensor[input_shape],Tensor[(7)])

        # Target model used to compute the target Q-values in training, updated
        # less frequently for increased stability.
        self._target_net = self._action_value_net.clone(CloneMethod.freeze)
        #self.d1_imus(np.array([1.,2.,2.]))
        # Function computing Q-values targets as part of the computation graph
        @Function
        @Signature(post_states0=Tensor[input_shape],post_states1=Tensor[7], rewards=Tensor[()], terminals=Tensor[()])
        def compute_q_targets(post_states0,post_states1, rewards, terminals):
            return element_select(
                terminals,
                rewards,
                gamma * reduce_max(self._target_net(post_states0,post_states1), axis=0) + rewards,
            )

        # Define the loss, using Huber Loss (more robust to outliers)
        @Function
        @Signature(pre_states0=Tensor[input_shape], pre_states1=Tensor[7], actions=Tensor[nb_actions],
                   post_states0=Tensor[input_shape], post_states1=Tensor[7], rewards=Tensor[()], terminals=Tensor[()])
        def criterion(pre_states0,pre_states1, actions, post_states0, post_states1, rewards, terminals):
            # Compute the q_targets
            q_targets = compute_q_targets(post_states0, post_states1, rewards, terminals)

            # actions is a 1-hot encoding of the action done by the agent
            q_acted = reduce_sum(self._action_value_net(pre_states0, pre_states1) * actions, axis=0)
...
def transform_input(responses):
    ...
    im_final = np.array(image.resize((42, 42)).convert('L')) 
...
def getDroneState():
    ...
    if collision_info == True:
        ...
        drone_collision = 0.0
    drone_state = np.array([quad_state.x_vel,quad_state.y_vel,quad_state.z_vel,
                            quad_vel.x_vel,quad_vel.y_vel,quad_vel.z_vel,drone_collision])
    return drone_state
...
img_state = transform_input(responses)
drone_state = getDroneState()
current_state = [img_state,drone_state]
